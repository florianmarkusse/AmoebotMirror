...
ShortcutBridgingAlg::ShortcutBridgingAlg() : Algorithm("Shortcut Bridging", "shortcutbridging") {
    addParameter("# Particles", "30");
    addParameter("Counter Max", "5");
};

void ShortcutBridgingAlg::instantiate(const int numParticles, const int counterMax) {
    if (numParticles <= 0) {
        emit log("# particles must be > 0", true);
    }
    else if (counterMax <= 0) {
        ...
        emit setSystem(std::make_shared<ShortcutBridgingSystem>(numParticles));
    }
}
