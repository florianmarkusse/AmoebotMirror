...
bool AmoebotParticle::canPush(int label) const
{
    ...
    return (isContracted() && hasNbrAtLabel(label) && nbrAtLabel<Particle>(label).isExpanded());
}
...
bool AmoebotParticle::canPull(int label) const
{
    ...
    return (isExpanded() && hasNbrAtLabel(label) && nbrAtLabel<Particle>(label).isContracted());
}
...
{
    return hasNbrAtLabel(label) && (nbrAtLabel<Particle>(label).head == nbrNodeReachedViaLabel(label));
}
